...
DI.PW=cpp
DI.PK=id
DI.SQL=select id, AuthorName AUTHOR, AuthorUrl AUTHOR_URL, WeiboText DRECONTENT, DATE_FORMAT(FROM_UNIXTIME(PublishTime/1000), '%Y-%m-%d %H:%i:%S') PUBLISHDATE, ForwardCount FORWARD_COUNT, CommentCount COMMENT_COUNT, Website SOURCEINFO, KeyMd5 DREREFERENCE from Results_sohu where Flag=1 limit 0, 1000
DI.UPDATE_SQL=update Results_sohu set Flag=0
EO.EMPTY={"退出":"EI1"}
